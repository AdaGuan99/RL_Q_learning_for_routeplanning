...
from b1maze_env_control import Maze
...
def update(order):
    ...
    for episode in range(600):  # 进行一百个回合

        if RL1.over == 1 and RL2.over == 1:  # 判断是否结束
            break
        # initial observation
        ...
        while True:
            ...
            if done1 != 1 and done2 != 1:
                ...
                # observation1_, reward1, done1 = env.step(action1, 1)
                # observation2_, reward2, done2 = env.step(action2, 2)
                observation1_, observation2_, reward, done1, done2 = env.step2(action1, action2)

                # RL learn from this transition
                RL1.learn(str(observation1), action1, reward, str(observation1_))
                RL2.learn(str(observation2), action2, reward, str(observation2_))
                ...
                action2 = RL2.choose_action(str(observation2), episode)
                observation2_, reward2, done2 = env.step1(action2, 2)
                RL2.learn(str(observation2), action2, reward2, str(observation2_))
                observation2 = observation2_

            else:
                action1 = RL1.choose_action(str(observation1), episode)
                observation1_, reward1, done1 = env.step1(action1, 1)
                RL1.learn(str(observation1), action1, reward1, str(observation1_))
                observation1 = observation1_
